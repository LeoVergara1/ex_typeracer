...
REM Realizado por: VMRB, 05  de Septiembre  de 2019
*/

create or replace PACKAGE        genpub.pc_comisiones_li AS
...
END pc_comisiones_li;
/

create or replace PACKAGE BODY        genpub.pc_comisiones_li AS
  ...
  CURSOR c_asignacion_descuentos (cp_pidm NUMBER, cp_periodo VARCHAR2, cp_detail_code VARCHAR2) IS
    ...
    CURSOR c_porcentaje_comision (cp_campus VARCHAR2) IS
      SELECT comision_ejecutivo/100 comision_ejecutivo FROM COMISIONLI.admin_de_comisiones WHERE campus_code = cp_campus;


    CURSOR c_porcentaje_comision_coor (cp_campus VARCHAR2) IS
      SELECT comision_coordinacion/100 comision_ejecutivo_coor FROM COMISIONLI.admin_de_comisiones WHERE campus_code = cp_campus;
  ...
  FUNCTION fn_monto_descuentos (p_pidm IN NUMBER, p_periodo IN VARCHAR2)RETURN NUMBER AS
    ...
    BEGIN
        ...
        v_consulta :=  'SELECT DISTINCT
                               ...
                               (select distinct puesto_promotor from COMISIONLI.asociacion_promotor where id_promotor = sovarol_id) puesto,
                               ...
                               pc_comisiones_li.fn_monto_pago_inicial(sgbstdn_pidm, sovlcur_term_code) pago_inicial,
                               pc_comisiones_li.fn_monto_descuentos(sgbstdn_pidm, sovlcur_term_code) total_descuentos,
                               pc_comisiones_li.fn_comision(sgbstdn_pidm, sovlcur_term_code) comision,
                               sovlcur_term_code periodo,
                               pc_comisiones_li.fn_fecha_filtro(sgbstdn_pidm, sovlcur_term_code) fecha_de_pago,
                               (select distinct id_coordinador from COMISIONLI.asociacion_promotor where id_promotor = sovarol_id and relacion_activa = ''Y'') id_coordinador,
                               (select distinct apellidos_coordinador||'' ''||nombre_coordinador  from COMISIONLI.asociacion_promotor where id_promotor = sovarol_id and relacion_activa = ''Y'') nombre_coordinador,
                               pc_comisiones_li.fn_comision_coordinador(sgbstdn_pidm, sovlcur_term_code) comision_coordinador,
															 (SELECT sum(tbraccd_amount) tbraccd_amount
                                    FROM tbraccd
                     ...
                     order by tbbestu_exemption_priority asc) where tbbestu_pidm = sovlcur_pidm and tbbexpt_term_code = sovlcur_term_code) discount_percent
                          ...
                          left join COMISIONLI.asociacion_promotor alias_t on id_promotor = sovarol_id and relacion_activa = ''Y''
                          WHERE srb.srbrecr_admin_seqno = (SELECT MAX(b.srbrecr_admin_seqno) FROM srbrecr b
                           ...
                           AND NOT EXISTS (SELECT * FROM COMISIONLI.autorizacion_comisiones
                           ...
                           AND pc_comisiones_li.fn_fecha_filtro(sgbstdn_pidm, sovlcur_term_code) >= :p_fecha_pago_ini
                           AND pc_comisiones_li.fn_fecha_filtro(sgbstdn_pidm, sovlcur_term_code) <= :p_fecha_pago_fin
